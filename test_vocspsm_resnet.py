...
NUM_STEPS =1449 #1449 # Number of images in the validation set.
RESTORE_FROM ='./model/resnet/model.resnet' #'./deeplab_resnet.ckpt'
...
def main():
    ...
    with tf.variable_scope('', reuse=False):
        net = DeepLabResNetModel({'input': image_batch}, num_classes=21,is_training=False)
        #net = DeeplabVGGASSPModel({'input': image_batch}, num_classes=21, is_training=False)
    with tf.variable_scope('', reuse=True):
        net075 = DeepLabResNetModel({'input': image_batch075}, num_classes=21,is_training=False)
    with tf.variable_scope('', reuse=True):
        net05 = DeepLabResNetModel({'input': image_batch05}, num_classes=21,is_training=False)
