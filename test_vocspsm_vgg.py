...
RESTORE_FROM ='./model/vgg/model.vgg' #'./deeplab_resnet.ckpt'
...
def main():
    ...
    with tf.variable_scope('', reuse=False):
        net = DeepLabVGGModel({'input': image_batch}, num_classes=21,is_training=False)
        #net = DeeplabVGGASSPModel({'input': image_batch}, num_classes=21, is_training=False)
    with tf.variable_scope('', reuse=True):
        net075 = DeepLabVGGModel({'input': image_batch075}, num_classes=21,is_training=False)
    with tf.variable_scope('', reuse=True):
        net05 = DeepLabVGGModel({'input': image_batch05}, num_classes=21,is_training=False)
